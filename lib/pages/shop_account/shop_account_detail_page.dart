...
class _ShopAccountDetailPageState extends State<ShopAccountDetailPage> {
  ...
  ShopAccountBloc bloc = ShopAccountBloc(ShopAccountLoading());
        ...
        children: <Widget>[
          Container(
            ...
            decoration: BoxDecoration(
                ...
                image: DecorationImage(image: CachedNetworkImageProvider('${shopAccountModel.coverImage}'), fit: BoxFit.cover)),
          ),
          Align(
            ...
            child: CircleAvatar(
              ...
              radius: 50,
              child: CircleAvatar(
                  radius: 40, backgroundColor: FOREGROUND_COLOR, backgroundImage: CachedNetworkImageProvider('${shopAccountModel.image}')),
            ),
          )
        ],
      ),
    ...
    return BlocBuilder(
        ...
        builder: (_, state) {
          if (state is ListProductsLoaded) {
            var products = state.products.where((product) => product.shopUsername == widget.shopAccountUsername).toList();
            return Container(
