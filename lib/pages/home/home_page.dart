...
import 'package:ankiishopii/widgets/shop_item.dart';
...
class _HomePageState extends State<HomePage> {
  ...
  ShopAccountBloc shopsForYouBloc = ShopAccountBloc(ShopAccountLoading())..add(GetShopsForYou());
  double topBarHeight = kToolbarHeight + 150;
  bool _isScrollToAppBar = false;

  _refresh() {
    productForYouBloc.add(GetProductsForYou());
    shopsForYouBloc.add(GetShopsForYou());
  }
...
//              - ScreenHelper.getPaddingTop(context) - kToolbarHeight;
                  ...
                  child: Row(
                    ...
                    children: <Widget>[
                      ...
                      CustomOnTapWidget(
                          ...
                          child: CartWidget(_isScrollToAppBar ? _cartIconKey : null))
                    ],
                  ),
                ),
              ),
          ...
          Container(
            ...
            decoration: BoxDecoration(
                ...
                image: DecorationImage(
                    image: CachedNetworkImageProvider("https://indochinapost.com/wp-content/uploads/f.jpg"), fit: BoxFit.cover)),
          ),
          Positioned.fill(
            top: topBarHeight - 30,
            child: Card(
                      ...
                      Expanded(
                        ...
                        child: TextField(
                          decoration: InputDecoration(
                              contentPadding: EdgeInsets.only(left: 5, right: 5, bottom: 5), hintText: 'Search', border: InputBorder.none),
                        ),
                      )),
                      CustomOnTapWidget(
                        onTap: () {
  ...
  Widget buildShops() {
    return BlocBuilder(
        cubit: shopsForYouBloc,
        builder: (context, state) {
          print("=======");
          print(state);
          if (state is ListShopsLoaded) {
            return Container(
              margin: EdgeInsets.only(top: _topBottomMargin, bottom: _topBottomMargin),
              height: 250,
              child: Column(
                  ...
                  Expanded(
                      child: ListView(
                          ...
                          children: state.shops
                              .map<Widget>((shop) => Container(
                                    margin: EdgeInsets.only(left: 10, bottom: 20),
                                    child: CustomShopGridItem(
                                      elevation: 5,
                                      cartIconKey: _cartIconKey,
                                      backgroundColor: FOREGROUND_COLOR,
                                      shop: shop,
          ...
          } else if (state is ShopAccountLoadingError) {
            return Center(
              child: CustomErrorWidget(),
            );
          } else {
