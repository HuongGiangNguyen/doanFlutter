...
class FavoriteService extends BlocService<FavoriteModel> {
  ...
  Future _updateCurrentAccountFavorite(FavoriteModel favorite) async {
    var currentFavorite = currentLogin.account.favorite.firstWhere((favoriteTemp) => favoriteTemp.id == favorite.id, orElse: () => null);
  ...
  //   FavoriteModel getFavoriteFromLocalByShopUsername(String username) {
  //   // print(currentLogin.account.favorite.map((e) => e.productId).toList());
  //   if (currentLogin == null) {
  //     return null;
  //   }
  //   return currentLogin.account.favorite.firstWhere((favorite) => favorite.username == username, orElse: () => null);
  // }
}
