...
class ProductService extends BlocService<ProductModel> {
  ...
  Future<List<ProductModel>> getAll({int from = 0, int limit}) async {
    ...
    var url = PRODUCT_ENDPOINT + '?${getQueryString({'from': from, 'limit': limit})}';
    print(url);
  ...
  Future<List<ProductModel>> getAllWithCategoryId(int categoryID, {int from = 0, int limit}) async {
    // TODO: implement getAll
    var rs = await HttpHelper.get(PRODUCT_ENDPOINT + "?categoryID=$categoryID" + '&${getQueryString({'from': from, 'limit': limit})}');
    if (rs.statusCode == 200) {
      var jsonList = jsonDecode(rs.body) as List;
      // print(jsonList.toList());
      var products = jsonList.map((j) => ProductModel.fromJson(j)).toList();
  ...
  Future<List<ProductModel>> searchProduct(String keyword, {int limit, int from = 0}) async {
    if (currentLogin != null) {
      var rs = await HttpHelper.post(PRODUCT_ENDPOINT + "/search?${getQueryString({'keyword': keyword, 'from': from, 'limit': limit})}", {},
